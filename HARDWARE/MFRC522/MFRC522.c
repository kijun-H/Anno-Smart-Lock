...
{ 
	...
	GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_15;						//RST引脚
	GPIO_Init(GPIOG, &GPIO_InitStructure);
	
	GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_6;							//SCK引脚
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	
	GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_6;							//NSS引脚
	GPIO_Init(GPIOE, &GPIO_InitStructure);
	
	GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_9;							//MOSI引脚
	GPIO_Init(GPIOC, &GPIO_InitStructure);
	
	
	GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_IN;			 			//输入模式
	GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_7;							//MISO引脚
	GPIO_Init(GPIOC, &GPIO_InitStructure);										
}

//模式0的收发一体函数
uint8_t  SPI_SendByte(uint8_t Data)  //模式0   MSB 
{
	uint8_t recv = 0;  //用于存储接收到的数据
	int i = 0;
